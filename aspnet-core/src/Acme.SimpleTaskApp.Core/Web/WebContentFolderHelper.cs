...
namespace Acme.SimpleTaskApp.Web
{
    ...
    public static class WebContentDirectoryFinder
    {
        public static string CalculateContentRootFolder()
        {
            ...
            var webMvcFolder = Path.Combine(directoryInfo.FullName, "src", "Acme.SimpleTaskApp.Web.Mvc");
            if (Directory.Exists(webMvcFolder))
            {
                return webMvcFolder;
            }

            var webHostFolder = Path.Combine(directoryInfo.FullName, "src", "Acme.SimpleTaskApp.Web.Host");
